...
void* FindRealAddress(PCSTR name)
{
	...
	{
		CHAR path[MAX_PATH];
		GetSystemDirectoryA(path, ARRAYSIZE(path));
		strcat_s(path, "\\version.dll");
		s_realVersionModule = LoadLibraryA(path);
	}
...
zzVerFindFileA(
	...
	__out_ecount(*puCurDirLen)   LPSTR szCurDir,
	_Inout_                      PUINT puCurDirLen,
	__out_ecount(*puDestDirLen)  LPSTR szDestDir,
...
DWORD APIENTRY zzVerFindFileW(
	...
	__out_ecount(*puCurDirLen)   LPWSTR szCurDir,
	_Inout_                      PUINT puCurDirLen,
	__out_ecount(*puDestDirLen)  LPWSTR szDestDir,
	_Inout_                      PUINT puDestDirLen
) {
...
DWORD APIENTRY zzVerInstallFileA(
	...
	__out_ecount(*puTmpFileLen)  LPSTR szTmpFile,
...
DWORD APIENTRY zzVerInstallFileW(
	...
	__out_ecount(*puTmpFileLen)  LPWSTR szTmpFile,
	_Inout_                      PUINT puTmpFileLen
) {
...
zzVerQueryValueA(
	...
	__deref_out_xcount(_Inexpressible_("buffer can be PWSTR or DWORD*")) LPVOID* lplpBuffer,
	_Out_ PUINT puLen
) {
